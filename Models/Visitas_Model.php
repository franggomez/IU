...
class LoteriaIU_Model { //declaración de la clase

    var $idVisita; // declaración del atributo identificador de visita
    var $fechaVisita; // declaración del atributo fecha inicial
  l
	var $informeVisita;// declaración del atributo informe de la visita
	var $estado;// declaración del atributo estado
	var $idCentro;// declaración del atributo identificador de centro
	var $idContrato;// declaración del atributo identificador de contrato

    var $mysqli; // declaración del atributo manejador de la bd


//Constructor de la clase
    function __construct($idVisita,$fechInicio,$fechFin,$informeVisita
	,$estado,$idCentro,$idContrato){

        //asignación de valores de parámetro a los atributos de la clase
        $this->idVisita = $idVisita;
        $this->fechaVisita = $fechInicio;
...
//existe ya en la tabla
    function ADD()
    {
        ...
        if (($this->idVisita <> '')){ // si el atributo clave de la entidad no esta vacio

            // construimos el sql para buscar esa clave en la tabla
            $sql = "SELECT * FROM VISITAS WHERE (idVisita = '$this->idVisita')";
            ...
            if (!$result = $this->mysqli->query($sql)){ // si da error la ejecución de la query
                ...
                if ($result->num_rows == 0){ // miramos si el resultado de la consulta es vacio (no existe el login)
                    ...
                    $sql = "INSERT INTO VISITAS (idVisita,fechaVisita,informeVisita,
					estado,idCentro,idContrato)
				VALUES ('".$this->idVisita."','".$this->fechaVisita."', '".$this->informeVisita."'
				, '".$this->estado."' , '".$this->idCentro."'
				, '".$this->idContrato."')";
...
// hace una búsqueda en la tabla con los datos proporcionados. Si van vacios devuelve todos
    function SEARCH()
    {
        ...
        $sql = "SELECT *
       			from VISITAS
				where idVisita LIKE '%".$this->idVisita."%' AND
						fechaVisita LIKE '%".$this->fechaVisita."%' AND
						informeVisita LIKE '%".$this->informeVisita."%'
						AND estado LIKE '%".$this->estado."%'
						AND idCentro LIKE '%".$this->idCentro."%'
						AND idContrato LIKE '%".$this->idContrato."%'";
...
// se manda un mensaje de que ese valor de clave no existe
    function DELETE()
    {
        ...
        if ($result->num_rows == 1) // si existe una tupla con ese valor de clave
        {
            ...
            $sql = "DELETE FROM VISITAS WHERE (idVisita = '".$this->idVisita."')";
    ...
    function EDIT()
    {
        ...
        if ($result->num_rows == 1) // si el numero de filas es igual a uno es que lo encuentra
        {
            ...
            $sql = "UPDATE VISITAS SET
					...
					informeVisita = '".$this->informeVisita."',
					estado = '".$this->estado."',
