...
class LoteriaIU_Model { //declaración de la clase
	...
	var $participacion;// declaración del atributo apellidos
	var $resguardo;// declaración del atributo apellidos
	var $ingresado;// declaración del atributo apellidos
	var $premiopersonal;// declaración del atributo apellidos
	var $pagado;// declaración del atributo pagado
    var $mysqli; // declaración del atributo manejador de la bd


//Constructor de la clase
    function __construct($email,$nombre,$apellidos,$participacion
	,$resguardo,$ingresado,$premiopersonal,$pagado){
        ...
        if (($this->email <> '')){ // si el atributo clave de la entidad no esta vacio
            ...
            if (!$result = $this->mysqli->query($sql)){ // si da error la ejecución de la query
                ...
                if ($result->num_rows == 0){ // miramos si el resultado de la consulta es vacio (no existe el login)
                    ...
                    $sql = "INSERT INTO `LoteriaIU` (`lot.email`,`lot.nombre`, `lot.apellidos`,
					`lot.participacion`,`lot.resguardo`,`lot.ingresado`, `lot.premiopersonal`
					,`lot.pagado`)
				VALUES ('".$this->email."','".$this->nombre."', '".$this->apellidos."'
				, '".$this->participacion."' , '".$this->resguardo."'
				, '".$this->ingresado."' , '".$this->premiopersonal."'
				, '".$this->pagado."');";
    ...
    function SEARCH()
    {
        ...
        $sql = "SELECT *
				...
				where `lot.email` LIKE '%".$this->email."%' AND
						...
						AND `lot.participacion` LIKE '%".$this->participacion."%'
						AND `lot.resguardo` LIKE '%".$this->resguardo."%'
						AND `lot.ingresado` LIKE '%".$this->ingresado."%'
						AND `lot.premiopersonal` LIKE '%".$this->premiopersonal."%'
						AND `lot.pagado` LIKE '%".$this->pagado."%'";
